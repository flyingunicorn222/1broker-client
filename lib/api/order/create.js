...
module.exports = function(config, params, callback) {
  var is_percent, max, stop_loss, take_profit;
  ...
  if (stop_loss) {
    if ((stop_loss != null ? stop_loss[0] : void 0) === '-' || stop_loss[0] === '+') {
      stop_loss = stop_loss.substr(1);
    }
  }
  if (take_profit) {
    if (take_profit[0] === '-' || take_profit[0] === '+') {
  ...
  if (params.order_type === 'market') {
    return call(config, 'market/quotes', {
      ...
      return console.log('calling with params ->', params);
    });
  }
};
